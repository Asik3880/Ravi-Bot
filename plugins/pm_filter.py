...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, SPRT_CHNL, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, HOW_TO_VERIFY, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, STREAM_BIN, STREAM_URL
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        btn = [            
            [InlineKeyboardButton("✅sᴇɴᴅ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴘᴛ ʜᴇʀᴇ✅", url="t.me/Filmymod7")],
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("I Lɪᴋᴇ Yᴏᴜʀ Sᴍᴀʀᴛɴᴇss Bᴜᴛ Dᴏɴ'ᴛ Bᴇ Oᴠᴇʀ Sᴍᴀʀᴛ, Fɪʀsᴛ Jᴏɪɴ Oᴜʀ Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ ᴅᴜᴅᴇ ! 😒", show_alert=True)
            return
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("🌿 Mᴏᴠɪᴇ Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🥀", url="telegram.me/FilmymodMovies")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        await query.message.edit_text(
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://telegra.ph/file/21200120f5f7806f5ba03.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="⋘ Pʟᴇᴀsᴇ Wᴀɪᴛ ⋙"
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("🌼 Mᴏᴠɪᴇ Cʜᴀɴɴᴇʟ🌼", url='https://t.me/+ym9eEFNVD7ZjOTk9')
                ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            m=await message.reply_sticker('CAACAgUAAxkBAAEKXlZlDp39vU5zKMOQE_e4vWSbsg1RngACoQADyJRkFOU68dHdpF6XMAQ', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
